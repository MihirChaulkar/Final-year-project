...
with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
  while cap.isOpened():
    ...
    if len(lmList) != 0:
      cv2.circle(image, (lmList[14][1], lmList[14][2]), 20, (0, 0, 255), cv2.FILLED)
      cv2.circle(image, (lmList[13][1], lmList[13][2]), 20, (0, 0, 255), cv2.FILLED)
      cv2.circle(image, (lmList[14][1], lmList[14][2]), 20, (0, 0, 255), cv2.FILLED)
      cv2.circle(image, (lmList[13][1], lmList[13][2]), 20, (0, 0, 255), cv2.FILLED)

      if (lmList[14][2] and lmList[13][2] >= lmList[16][2] and lmList[15][2]):
      ...
      if (lmList[14][2] and lmList[13][2] <= lmList[16][2] and lmList[15][2]) and stage == "down":
        stage = "up"
        counter += 1
